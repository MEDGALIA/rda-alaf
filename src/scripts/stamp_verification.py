...
Writes into a checkout the caller has already prepared as the `auto-sync`
branch, never into `main` directly: `main` requires a PR, and this repo has
no way to grant a bot write access to a protected branch (tested: the
built-in Actions app cannot be added to `bypass_pull_request_allowances`;
only a self-created GitHub App can, which needs manual one-time setup --
see drafts/VANTAGE-Tech-Radar-Sync-Plan.md, Step 4). Pushing to an
unprotected branch instead needs no new credentials -- the default
GITHUB_TOKEN already has contents: write in this repo. An admin merges
`auto-sync` into `main` whenever convenient; their existing bypass covers
that merge the same as any other they do.
...
def main() -> None:
    ...
    parser.add_argument("--write-dir", type=Path, required=True, help="data/json inside the auto-sync checkout")
